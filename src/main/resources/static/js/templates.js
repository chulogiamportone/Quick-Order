...
async function mostrarPopup(a) {
  ...
  for (let pedido of pedidos.data) {
    if (pedido.id == a) { // Procesar solo el pedido seleccionado
        ...
        switch (user) {
          case "MOZO":
            // **[CORRECCIÓN]** Ahora llama a la nueva función Mozo (estilo moderno)
            popupContentHtml = generarPopupMozo(pedido); 
            break;
          case "CAJA":
            // **[CORRECCIÓN]** Ahora llama a la nueva función Caja (estilo moderno)
            popupContentHtml = generarPopupCaja(pedido); 
            break;
          case "COCINA":
            // **[CORRECCIÓN]** Ahora llama a la nueva función Cocina (estilo moderno)
...
function generarPopupMozo(pedido) {
    ...
    if (pedido.lista_Productos && pedido.lista_Productos.length > 0) {
        productosHtml = pedido.lista_Productos.map(item => {
            // Aseguramos que precio y cantidad sean numéricos para el cálculo
    ...
    // 2. Lógica de habilitación del botón de acción
    // El Mozo solo puede entregar si el estado es 'Listo para entregar'
    ...
    // 3. Devolver la estructura completa (Estilo moderno unificado)
    return `
...
function generarPopupCaja(pedido) {
    ...
    const disabled = entregado ? '' : 'disabled'; // SOLO habilitar si está "entregado"
...
function generarPopupCocina(pedido) {
    ...
    if (estadoActual.toLowerCase() === 'pendiente') { // Asumimos que el estado inicial es "pendiente" o "nuevo pedido"
        ...
        // Si es 'Listo para entregar', 'Entregado' o 'Cancelado', el botón se deshabilita
    ...
    // 1. Generar listado de productos (Nombre, Cantidad y Descripción - SIN PRECIOS)
    if (pedido.lista_Productos && pedido.lista_Productos.length > 0) {
        productosHtml = pedido.lista_Productos.map(item => `
            <div class="qo-summary-item d-flex align-items-center justify-content-between">
                <div class="item-info d-flex align-items-center">
    ...
    // 2. Devolver la estructura completa
    return `
        <div class="qo-popup-summary-content">
            <div class="popup-header">
                <h2 class="pop-pedido">Pedido N°${pedido.numero || pedido.id}</h2>
